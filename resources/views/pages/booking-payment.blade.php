        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 lg:col-span-1 border-2 border-gray-200/50 backdrop-blur-sm sticky top-24">
                <div class="space-y-6">
                    ...
                    <form action="{{ route('booking.fourth-booking') }}" method="POST" id="payment-form">
                        @csrf
                        <input type="hidden" id="id" name="id" value="{{ $booking->id }}">
                        <input type="hidden" id="payment_price" name="payment_price" value="{{ $booking->total_payment }}">
                        <button type="button" id="pay-button"
                            class="w-full bg-gray-400 text-white font-bold py-3 px-4 rounded-lg mt-6 flex items-center justify-center transition-colors duration-200 cursor-not-allowed"
                            disabled>
